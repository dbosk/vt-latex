...
Discernment requires experienced \emph{variation}: an aspect becomes
visible only when it varies against an invariant background.
...
observational basis: the qualitatively different ways learners experience
a phenomenon.
In this paper, as in the companion papers
\autocite{VTProgMisconceptions,VTDebug,VTTerminal}, documented
difficulties play the role of those phenomenographic observations; the
instruments of \cref{sec:method-instruments} add the students' own
accounts.
...
learners now discern them
\autocite[Ch.~7]{NCOL}\autocite{PangMarton2005,Kullberg2017ZDM}.
...
We also rely on the \emph{notional machine}: an abstract model of the
system that novices need in order to reason about what their actions do.
The literature identifies the notional machine as a major challenge in
programming education and argues it should be an explicit learning
objective \autocite{Sorva2013NotionalMachines,DuBoulay1981BlackBox};
novice difficulties are tied to inaccurate mental models more generally
\autocite{QianLehman2017Misconceptions,BenAri1998Constructivism}.
LaTeX needs a notional machine of the document: source text and typeset
output are distinct artifacts, related by a compiler that reads the whole
source (and sometimes needs several passes).
The WYSIWYG word processor, where document and screen coincide, is the
prior model this machine must displace.
% TODO(introtools#124, introtools#125): du Boulay and Ben-Ari full texts
% still to be verified before quoting specifics; see misconceptions.bib.
